  ...
  <div class="page free">
    <div class="container">
      ...
      <div class="articleBody">
        ...
        <p>You liked this post ? Try <router-link to="/free">this one</router-link>.</p>
      </div>

      <div v-if="!$testUser.premium" id="poool-widget"></div>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      async sendHit() {
        ...
        if (this.$testUser.logged && this.$testUser.premium) {
          window.poool('config', {
            force_widget: 'disabled',
            signature_enabled: false,
          })
        }

        window.poool('send', 'page-view', 'premium')
      },
      login() {
        this.sendHit()
      },
    ...
    mounted() {
      window.poool('event', 'onSubscribeClick',
    ...
    beforeRouteLeave(to, from, next) {
      window.poool('flush')
      window.poool('unevent', 'onSubscribeClick',
        this.onSubscribeClick.bind(this)
      );
      next()
    }
